...
class Metabox {
   ...
   * @return $this
   */
  function make($name = null, $settings = array()) {
    $this->check($name, 'Making Meta Box: You need to enter a name.');

    $this->label = $this->id = $name;
    $this->sanitize_string($this->id);
    if(empty($settings['callback'])) $settings['callback'] = array($this, 'meta_content');
    ...
    $defaults = array(
      ...
      'args' => array()); // arguments to pass into your callback function.
  ...
  function add_post_type($s) {
    if( is_string($s) ) {
      $this->merge($this->post_types, array($s));
